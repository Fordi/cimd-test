...
const configChanged = async () => {
  ...
  const { authorization_endpoint, token_endpoint } = authServer
  ...
  ep.searchParams.append(
    "client_id",
    "https://fordi.github.io/cimd-test/client.json",
  );
  ep.searchParams.append(
    "scope",
    scope.value
...
if (returned.state) {
  const [verifier, created, { tokenEndpoint, openidConfigUrl, scopes }] =
    JSON.parse(localStorage.getItem(`${STORAGE_KEY_ROOT}${returned.state}`));
  scope.value = scopes;
